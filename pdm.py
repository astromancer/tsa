...
class PyPDM(object):
  ...
  def __setUpEquiBlocks(self, nbins, phases):
    ...
    """
    blockBegin = (np.arange(nbins) / float(nbins)).tolist()
    blockEnd   = ((np.arange(nbins) + 1.0) / float(nbins)).tolist()
    Ns = []
    for i in range(nbins):
      indi = np.where(np.logical_and(phases >= blockBegin[i], phases < blockEnd[i]))[0]
      Ns.append(len(indi))
  ...
  def __setUpEquiBlocksCover(self, nbins, phases, covers):
    """
      ...
      Returns: A list with the beginnings and one list with the ends of the bins (in phase). 
    """
    blockBegin = np.arange(nbins) / float(nbins)
    blockEnd   = (np.arange(nbins) + 1.0) / float(nbins)
    
    blockBeginOrig = blockBegin.copy()
    blockEndOrig = blockEnd.copy()
    
    offset = 0
    for i in range(1,covers):
      offset = float(i)/float(nbins * covers)
      blockBegin = np.append(blockBegin, blockBeginOrig + offset)
      blockEnd   = np.append(blockEnd, blockEndOrig + offset)
  ...
  def __getThet(self, phase, mag, bbegin, bend):
    ...
    for i in range(len(bbegin)):
      # Points belonging to a chunk
      indi = np.where(np.logical_and(phase >= bbegin[i], phase < bend[i]))[0]
      nj = float(len(indi))
      if nj > 1:
  ...
  def pdmEquiBin(self, nbins, scanner):
    ...
    self.frequencies = 1.0 / np.array(periods)
  ...
  def pdmEquiBinCover(self, nbins, covers, scanner):
    ...
    for val in scanner:
      ...
      phase = np.append(p[indi], p[indi]+1.0)
      mag = np.append(self.y[indi], self.y[indi])
